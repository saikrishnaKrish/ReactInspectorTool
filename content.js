 ...
 * VERSION: 1.8.0 - JSX Snippet Export & Computed Style Explorer.
 */
...
(function() {
  ...
  const injectStyles = () => {
    ...
    style.textContent = `
      ...
      .ri-small-btn { font-size: 9px; padding: 2px 6px; border: 1px solid var(--ri-border); border-radius: 4px; cursor: pointer; background: var(--ri-bg); color: var(--ri-text-dim); }
      .ri-small-btn:hover { border-color: var(--ri-accent); color: var(--ri-accent); }
  ...
  const initUI = () => {
    ...
    searchOverlay.innerHTML = `<input type="text" id="comp-search" placeholder="Search Component Name..." style="width:100%;background:var(--ri-header-bg);border:1px solid var(--ri-border);color:var(--ri-text);padding:10px;border-radius:6px;outline:none;">`;
  ...
  const updatePanel = (targetFiber, element) => {
    ...
    const refs = targetFiber.ref || props.ref || null;
    ...
    sidePanel.innerHTML = `
      ...
      <div style="padding:16px;overflow-y:auto;flex:1;">
        ...
        <div style="margin-bottom:16px; display:flex; gap:6px;">
           <button class="ri-small-btn" id="btn-copy-jsx" style="background:var(--ri-accent); color:white; border:none; font-weight:bold;">Copy as JSX</button>
        ...
        <div class="ri-section-title">Styles & Classes</div>
        ${classes ? `
          <div class="ri-class-badge">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
              <div style="color:var(--ri-accent);font-weight:bold;font-size:9px;text-transform:uppercase;">Classes</div>
        ...
        <pre class="ri-code-block" style="padding:10px;border-radius:8px;font-size:10px;margin-bottom:16px;">${JSON.stringify(essentialStyles, null, 2)}</pre>
        ...
        <input type="text" id="prop-filter" class="ri-prop-filter" placeholder="Filter props or state...">

        <div class="ri-section-title">
          <span>Props</span>
          <div class="ri-btn-group">
            <button class="ri-small-btn" id="btn-map-props">Map to Console</button>
        ...
        ${state ? `
          <div class="ri-section-title">
            <span>State</span>
            <div class="ri-btn-group">
              <button class="ri-small-btn" id="btn-map-state">Map to Console</button>
    ...
    sidePanel.querySelector('#btn-copy-jsx').onclick = (e) => {
       const jsx = `<${name} ${Object.entries(props).filter(([k]) => k !== 'children' && typeof props[k] !== 'function').map(([k, v]) => `${k}={${JSON.stringify(v)}}`).join(' ')} />`;
       navigator.clipboard.writeText(jsx);
       const btn = e.currentTarget;
       btn.innerText = 'Copied!';
       setTimeout(() => btn.innerText = 'Copy as JSX', 1500);
    };
  ...
  window.addEventListener('keydown', (e) => {
    ...
    if (e.altKey && k === 'i') {
      isDevMode = !isDevMode;
      initUI();
      if (!isDevMode) { overlay.style.display = 'none'; sidePanel.style.display = 'none'; }
    }
    if (e.altKey && k === 's') {
      if (!isDevMode) return;
  ...
  console.log('%c React Inspector Pro 1.8.0 Loaded ', 'background:#3b82f6;color:white;font-weight:bold;padding:4px;');
})();